...
实测还踩到过：A-01 把两个房间**都**命名为「卧室」（没区分主卧/次卧）。
此时第二间房会既被判成重复、又不在 `unassigned_rooms` 里 —— **等于凭空消失**。

名字是上游给的，本 Agent 修不了，但必须**如实说出来**（写进 `data_gaps`，
措辞是「户型数据中有同名房间，系统无法区分」），因为用户看到"两间卧室"
却只拿到一间房的规划，得知道为什么。

注意这时**不算模型的账**：重复是上游误导造成的，不该记成"模型写重了"。
`data_gaps` 的措辞与 `based_on.rooms_total` vs `rooms_planned` 的差距，
两者合起来把这个情况讲清楚。
